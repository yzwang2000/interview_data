...
#### 反转链表
1. 反转头节点不需要使用虚拟头节点。
2. 使用 `双指针的方法`。fast 指针初始化为头节点, slow 指针初始化为 nullptr。循环逻辑就是, 用 tmp 临时保存 fast->next, 然后 fast->next 指向 slow，更新 slow = fast, fast=tmp, 直到 fast == nullptr, 此时返回链表头节点 slow。

#### 两两交换链表中的节点
1. 需要使用虚拟头节点。
2. 做链表的题目, 一定要把画图，用一组数据进行模拟, 把每个步骤清楚的标注出来, 依据标注出来的结果来写代码。
3. 注意边界条件, 根据到结尾的第几个节点终止来确定循环的边界条件。本题是 `while(cur->next != nullptr && cur->next->next != nullptr)`。

#### 删除链表的倒数第 N 个节点(这道题目也是代表了这种不能随机访问的数据结构的在第 i 个位置删除、插入、修改元素的方法)
1. 需要使用虚拟头节点。
...
```
2. 对于两个链表或者两个数组, 不能确定谁的元素多，谁的元素少时, 直接先判断长度, 然后用 `std::swap` 进行交换, 保证想要的那个是元素多的。
```C++
curA = headA;
curB = headB;
...
2. 快慢指针`都从头指针出发`, 快指针每次移动 2 步, 慢指针每次移动 1 步, 如果快指针等于慢指针, 则说明有环, 如果快指针检测到有 nullptr, 那就是无环。
...
#### 路径总和
* 利用回溯的思想（深度优先搜索）, 遇到叶子节点判断是否是路径和。这道题目的技巧有两点，第一点就是不用累加而用累减，判断剩余路径是否是剩余的累加和。第二点就是返回值是 bool, 判断是否构成了。
...
int main() {
    ...
    // 创建一个动态规划数组dp, 初始值为0
    vector<int> dp(N + 1, 0);  // dp[0] 的价值肯定是 0, 因为没给任何空间
...
##### 最长公共子数组
* 这道题目就是两个数组，找最长公共子数组。我们利用二维的 dp 数组来解题。
...
public:
    int numDistinct(string s, string t) {
        vector<vector<uint64_t>> dp(s.size() + 1, vector<uint64_t>(t.size() + 1));  // dp[i][j] 表示以 i-1 为结尾的 s 子序列中出现以 j-1 为结尾的 t 的个数。
...
##### 与岛屿有关的问题
* 一般来说，岛屿问题都需要`邻接矩阵存储地图`，并初始化一个与地图同样大小的 visited 数组来记录每个点是否访问过。
...
##### 字符串接龙
* 给一个 beginStr 和一个 endStr, 通过中间状态 strList 的变换转到最终 endStr 的状态。这里并没有初始建图，因为初始建图的目的其实也就是找到当前节点和那些节点相连。
...
int main() {
    ...
    // 第一次先寻找入度为 0 的节点, 之后为什么没有这个操作呢？因为之后出现入读为零的节点，一定都是与被删除节点连接的节点。
    queue<int> que;
    for (int i = 0; i < n; i++) {
...
##### Folyd 算法
* Folyd 是一种动态规划算法, 稠密图效果最佳。对于稠密图, 效率要高于执行 |V| 次的 Dijkstra 算法。时间复杂度较高, 不适合大量计算, 时间复杂度 O(n^3), 空间复杂度 O(n^2)。可以正确地处理有向图或无向图或负权图(但不存在负权回路)的最短路径问题。
```C++
...
int main()
{
    ...
    // D 的初始化, D[i][j] 表示节点 i 到节点 j 的直达距离。D[i][i] 为 0, 当 i 和 j 之间没有直达节点时, 初始化为 无穷大。
    // path 表示在 i 号节点到 j 号节点最短路径上的 j 号节点的前驱节点。 path[i][i] 为 i, 当 i 到 j 目前没有路径时, 标记为 0。
    ...
    // 每次解封一个新的节点个数, 都要更新 D 中的每一个值 和 path 中的每一个值
