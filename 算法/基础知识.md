...
##### Folyd 算法
* Folyd 是一种动态规划算法, 稠密图效果最佳。对于稠密图, 效率要高于执行 |V| 次的 Dijkstra 算法。时间复杂度较高, 不适合大量计算, 时间复杂度 O(n^3), 空啊金复杂度 O(n^2)。可以正确地处理有向图或无向图或负权图(但不存在负权回路)的最短路径问题。
```C++
...
int main()
{
    ...
    // D 的初始化, D[i][j] 表示节点 i 到节点 j 的直达距离。D[i][i] 为0, 当 i 和 j 之间没有直达节点时, 初始化为 无穷大。
